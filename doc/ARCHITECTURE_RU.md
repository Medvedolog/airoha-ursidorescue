...
Один модуль Go `ursidorescue` (Go 1.24). Ядро использует только стандартную библиотеку; единственные
внешние зависимости — pure-Go Bubble Tea / Lip Gloss / Bubbles для TUI (`tui*.go`), версии закреплены
в `go.mod` / `go.sum` (сознательное исключение, ТЗ §5.3). Платформенные различия вынесены в файлы с суффиксами `_linux.go` / `_windows.go`
...
| `payloads/md/vanilla-u-boot-md-0.1.0-alpha5-t67.fip`, `payloads/mf/vanilla-u-boot-mf-0.1.0-alpha5-t67.fip` | UrsusFlasher 0.2.67 `data/payloads/{md,mf}/vanilla/`; = `vanilla-u-boot.fip` UrsusBoot `8c2567ab` (t67), SHA256 из provenance; для «Эксперт → Возврат/обновление vanilla U-Boot» |
...
Нужен только Go ≥ 1.24 (модули Charm скачиваются через Go proxy при первой сборке). Никаких MinGW, windres, Python или ImageMagick.
...
- **TUI (`tui_test.go`):** каждый экран (меню, операция, подтверждение, выбор порта, крупный лог) ровно
  80×24 без переполнения строк; подтверждение и выбор через мост `tuiUI` (верная и неверная фраза, Esc,
  выбор стрелками); надписи СТОП по `CancelState`; операция из TUI останавливается по `s`; события во время
  терминала копятся и возвращаются; очистка ANSI и `\r` в выводе устройства.
