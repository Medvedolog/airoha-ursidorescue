...
func (a *App) physicalRestoreWizard() error {
	...
	if len(blbad) > 0 || len(bad) > 0 {
		return fmt.Errorf(L("восстановление physical image в 0.2.0-test8 требует отсутствия bad-блоков (bl2=%d ubi=%d); используйте восстановление с учётом формата", "physical-image restore 0.2.0-test8 requires zero bad blocks (bl2=%d ubi=%d); use a format-aware restore instead"), len(blbad), len(bad))
	}
	local, e := a.networkIP()
	if e != nil {
		return e
...
func (a *App) expertUBIVolume() error {
	...
	st, _ := os.Stat(path)
	if st.Size() > maxGenericRAMFile {
		return errors.New(L("файл volume >64 MiB за один раз не поддерживается в 0.2.0-test8", "expert one-shot volume file >64 MiB is not supported in 0.2.0-test8"))
	}
	sha, _ := shaFile(path)
