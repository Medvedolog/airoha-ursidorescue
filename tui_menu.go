	...
	return []tuiGroup{
		{L("Главное", "Main"), []tuiItem{
			...
			{label: L("Починить загрузку / заменить FIP", "Repair boot / replace the FIP"), hint: L("UBI-том fip", "UBI volume fip"), kind: "fip-repair",
				desc: L("OpenWrt не загружается, но UBI цел: перезаписывается только UBI-том fip (BL31 + U-Boot) — встроенным RAM FIP или вашим .fip, со сверкой CRC.\n\nНужно: UART и LAN.\n\nРиск: WRITE — подтверждение фразой WRITE FIP.",
					"OpenWrt does not boot but UBI is intact: only the UBI volume fip (BL31 + U-Boot) is overwritten, with the built-in RAM FIP or your .fip, and CRC-checked.\n\nNeeds: UART and LAN.\n\nRisk: WRITE; typed phrase WRITE FIP.")},
			...
			{label: L("Диагностика NAND / UBI / U-Boot", "NAND / UBI / U-Boot diagnostics"), hint: L("только чтение", "read-only"), kind: "diagnostics",
				desc: L("Только чтение: через RAM U-Boot показывает разметку MTD, bad-блоки, UBI-тома и версию U-Boot. Ничего не пишет.\n\nНужно: UART.",
					"Read-only: through the RAM U-Boot shows the MTD layout, bad blocks, UBI volumes and the U-Boot version. Writes nothing.\n\nNeeds: UART.")},
		...
		{L("Эксперт", "Expert"), []tuiItem{
			...
			{label: L("Диагностика", "Diagnostics"), hint: L("только чтение", "read-only"), kind: "diagnostics",
				desc: L("То же, что «Диагностика» в главном меню: только чтение через RAM U-Boot.", "Same as Diagnostics in the main menu: read-only through the RAM U-Boot.")},
