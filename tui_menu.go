...
func tuiMenu() []tuiGroup {
	return []tuiGroup{
		{L("Главное", "Main"), []tuiItem{
			{label: L("Восстановить заводскую Nokia", "Restore stock Nokia"), hint: "ERASE · mtd16 / all_flash", kind: "stock-restore",
			...
			{label: L("Восстановить FIP (UBI цел)", "Restore the FIP (UBI intact)"), hint: L("WRITE · только том fip", "WRITE · fip volume only"), kind: "fip-repair",
			...
			{label: L("Восстановить весь NAND", "Restore the full NAND"), hint: L("ERASE · образ 256 МиБ", "ERASE · 256 MiB image"), kind: "physical-restore",
			...
			{label: L("Загрузить ITB в RAM", "Boot an ITB from RAM"), hint: L("RAM only · без записи", "RAM only · no write"), kind: "itb-boot",
			...
			{label: L("Собрать пакет логов", "Build a log bundle"), hint: L("без устройства", "no device"), kind: "support-bundle",
		...
		{L("Эксперт", "Expert"), []tuiItem{
			...
			{label: L("Установить UrsusBoot (UART)", "Install UrsusBoot (UART)"), hint: "WRITE · MD / MF", kind: "ursusboot-install",
			...
			{label: L("Вернуть/обновить vanilla U-Boot", "Return/update vanilla U-Boot"), hint: "WRITE · UBI · MD / MF", kind: "vanilla-uboot",
			...
			{label: L("Записать UBI-том из файла", "Write a UBI volume from a file"), hint: L("WRITE · UBI-том", "WRITE · UBI volume"), kind: "ubi-volume",
